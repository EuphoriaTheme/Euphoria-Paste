...
class DocumentHandler {
  constructor(options = {}) {
    ...
    if (this.enableDiscordLogging && this.discordWebhookUrl) {
      logger.info('Discord webhook integration enabled in DocumentHandler');
    } else if (this.enableDiscordLogging && !this.discordWebhookUrl) {
  ...
  buildErrorBody(request, message) {
    return {
      message,
  ...
  async sendLogToDiscord(message) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Discord webhook error: ${res.status} - ${res.statusText} - ${errorText}`,
        );
      }
    ...
    try {
      ...
      if (data) {
        ...
        if (request.method === 'HEAD') {
          ...
          response.end(
            JSON.stringify({ data, key, requestId: request.requestId || null }),
          );
        }
      } else {
        ...
        if (request.method === 'HEAD') {
          ...
          response.end(
            JSON.stringify(this.buildErrorBody(request, 'Document not found.')),
          );
        }
      }
    } catch (error) {
      ...
      response.end(
        JSON.stringify(
          this.buildErrorBody(request, 'Error retrieving document.'),
        ),
      );
    }
  }
    ...
    try {
      ...
      if (data) {
        ...
        if (request.method === 'HEAD') {
          ...
          response.end(
            JSON.stringify(
              this.buildErrorBody(request, 'Raw document not found.'),
            ),
          );
        }
      }
  ...
  async handlePost(request, response) {
    ...
    const onSuccess = async () => {
      if (this.maxLength && buffer.length > this.maxLength) {
        ...
        response.end(
          JSON.stringify(
            this.buildErrorBody(request, 'Document exceeds maximum length.'),
          ),
        );
        return;
      }
      ...
      if (!buffer || !buffer.trim()) {
        ...
        response.end(
          JSON.stringify(
            this.buildErrorBody(request, 'Document body cannot be empty.'),
          ),
        );
        return;
      }
      ...
      try {
        ...
        if (result) {
          ...
          await this.sendLogToDiscord(
            `:white_check_mark: A new document was created: ${documentUrl}`,
          );
          response.writeHead(200, { 'content-type': 'application/json' });
          response.end(
            JSON.stringify({ key, requestId: request.requestId || null }),
          );
        } else {
        ...
        response.end(
          JSON.stringify(
            this.buildErrorBody(request, 'Error adding document.'),
          ),
        );
      }
    };
    ...
    if (ct && ct.split(';')[0] === 'multipart/form-data') {
      ...
      busboy.on('field', (fieldname, val) => {
        if (fieldname === 'data') {
          ...
          if (this.maxLength && buffer.length > this.maxLength) {
            ...
            response.end(
              JSON.stringify(
                this.buildErrorBody(request, 'Payload too large.'),
              ),
            );
          }
        }
      ...
      busboy.on('error', (error) => {
        logger.error(`Upload parsing error: ${error.message}`);
        if (!response.headersSent) {
          response.writeHead(400, { 'content-type': 'application/json' });
          response.end(
            JSON.stringify(
              this.buildErrorBody(request, 'Malformed multipart request.'),
            ),
          );
        }
      });
      ...
      request.on('data', (data) => {
        ...
        if (this.maxLength && buffer.length > this.maxLength) {
          ...
          response.writeHead(413, { 'content-type': 'application/json' });
          response.end(
            JSON.stringify(this.buildErrorBody(request, 'Payload too large.')),
          );
          request.destroy();
        }
      });
      ...
      request.on('error', (error) => {
        ...
        response.writeHead(500, { 'content-type': 'application/json' });
        response.end(
          JSON.stringify(this.buildErrorBody(request, 'Connection error.')),
        );
        cancelled = true;
      });
    }
